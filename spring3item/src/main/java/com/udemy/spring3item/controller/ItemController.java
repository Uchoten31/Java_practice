...
public class ItemController {
	...
	/*
	 *  itemServiceのgetItemがOptionalオブジェクトになったので、
	 *  .orElseThrowで、nullの場合、例外クラスをThrowするように変更
	 *  -> インスタンス化させたものを設定
	 *  -> この例外は、ControllerAdviceクラスがハンドリングして、エラーメッセージを自動で返す
	 *  -> その実装は改めてこちらでする必要はなく、自動でクライアントに伝搬される
	 *  -> そのためControllerクラスでは自作のExceptionをThrowするだけでよい
	 */
